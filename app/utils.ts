...
import { RecordType, RecordTypeString, type DNSAnswerType, type ENCODED_LABELS } from './types'
...
export function encodeDomainName(
  ...
  if (index === -1 || index > 0) {
    const remainingLabelSequence = index > 0 ? domainName.slice(0, index) : domainName
...
export function decodeDomainName(
  ...
  while (startIndex < buffer.length) {
    ...
    if (isPointer(buffer, startIndex)) {
      const pointerOffset = decodePointer(buffer.subarray(startIndex, startIndex + 2))
      ...
      labels.push(buffer.subarray(startIndex + 1, startIndex + 1 + labelLength).toString('latin1'))
...
export function decodePointer(pointerBuffer: Buffer): number {
  ...
  const pointerBinary = pointerBuffer.readUInt16BE(0).toString(2).padStart(16, '0')
...
export function encodeRecords(
  ...
  for (const record of records) {
    ...
    encodedRecords = Buffer.concat([new Uint8Array(encodedRecords), new Uint8Array(encodedRecord)])
