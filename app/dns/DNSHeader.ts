...
export class DNSHeader extends BaseDNSComponent<DNSHeaderType> {
  ...
  encode(): Buffer {
    try {
      ...
      throw new Error(`Failed to encode DNSHeader: ${(e as Error).message}`)
    }
  }
  ...
  static encodeRaw(data: DNSHeaderType): Buffer {
    try {
      if (!data || typeof data !== 'object') {
        throw new Error('Invalid data: data must be a DNSHeaderType object')
      }
      return new DNSHeader(data).encode()
    } catch (e) {
      throw new Error(`Failed to encode raw DNS header: ${(e as Error).message}`)
    }
  }
  ...
  static decode(buffer: Buffer, offset: number = 0): { result: DNSHeader; nextOffset: number } {
    try {
      ...
      throw new Error(`Failed to decode DNSHeader: ${(e as Error).message}`)
    }
  }
}
